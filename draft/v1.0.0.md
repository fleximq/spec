...
| PUB   | Topic        | 5     | Publish a message to a topic            | `topic`, `status`                |
...
4.  **Disconnection**:
    ...
    - Connections can be terminated due to errors, timeouts, or network issues.
...
- **408 Request Timeout**: The client did not produce a request within the time that the server was prepared to wait. (Less common in this protocol, usually transport handles timeouts). 
...
- **STDIO**: `stdio://`
  - This scheme indicates that the client will communicate with the broker over its standard input and standard output streams. It's typically used when the broker process launches and directly communicates with client subprocesses.
...
- **SUB and UNSUB Messages**: Must have a `topic` field in the Header. They must not contain `routing`, `reqrep`, `status`, or `auth` fields.
...
- **Connection Timeout**: If a connection is idle for an extended period, it may be terminated by either party. Implementations should handle abrupt disconnections gracefully.
...
### 7.5. Connection Keep-alive

Connection keep-alive in fleximq is handled by the underlying transport layer rather than at the protocol level. This approach leverages the native keep-alive mechanisms provided by different transport protocols:

- **TCP Keep-alive**: When using TCP as the transport layer, connection health is maintained through TCP's built-in keep-alive mechanism. This includes TCP keep-alive probes sent at the operating system level.

- **WebSocket Keep-alive**: When using WebSocket as the transport layer, connection health is maintained through WebSocket ping/pong frames as defined in RFC 6455, or through the underlying TCP keep-alive mechanism.

- **Other Transport Protocols**: For other transport protocols, their respective native keep-alive mechanisms should be utilized.

This design decision simplifies the fleximq protocol by delegating connection health monitoring to the transport layer, which is better positioned to handle network-level connectivity issues. Applications can configure transport-specific keep-alive parameters (such as keep-alive intervals and timeouts) according to their requirements.
